...
def visualiseRoute(data, pathGiven):
    ...
    # Find min and max latitudes and longitudes to scale coordinates
    ...
    def lat_lon_to_screen(lat, lon):
        # Normalize latitude and longitude to fit the screen
        norm_x = (lon - min_lon) / (max_lon - min_lon)
        norm_y = (lat - min_lat) / (max_lat - min_lat)
        screen_x = norm_x * (screen.get_width() - 100) + 50  # Add some padding
        screen_y = (1 - norm_y) * (screen.get_height() - 100) + 50  # Invert y to match screen coordinates
    ...
    def draw_all_stations():
        for station, details in data.items():
            ...
            # Draw station circle
            pygame.draw.circle(screen, (200, 200, 200), pos, 5)  # Light grey for other stations
            # Optionally, draw station names
            font = pygame.font.Font(None, 8)
            text = font.render(station, True, (128, 128, 128))  # Dark grey for names
    ...
    while running:
        ...
        color = (255, 0, 0)
        previous_line = None
        for i, item in enumerate(pathGiven[:-1]):  # Exclude the last item for now
            ...
            if line != previous_line and previous_line is not None:
                text = font.render(station_from, True, (0, 0, 0))
                screen.blit(text, (from_pos[0] + 15, from_pos[1]))
                pygame.draw.circle(screen, pygame.Color(0,0,255), from_pos, 10)  # Example: White ring for line change

            size = 5
            color = (20, 20, 20)
            previous_line = line

        # Handle the last station properly
        if pathGiven:
            last_station = pathGiven[-1] if isinstance(pathGiven[-1], str) else pathGiven[-1][0]
            if last_station in data:  # Check if the last station is in data
...
url = "http://127.0.0.1:5000/api/findRoute/Park Royal/Canning Town"
response = requests.get(url)
if response.status_code == 200:
    route_info = response.json()
    # Assuming route_info structure is { "cost": XX, "path": [...] }
